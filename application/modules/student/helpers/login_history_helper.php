...
{
    $ci =& get_instance();
    $browsers = $ci->db->select('browser, COUNT(id) as total')->group_by('browser')->get('student_logs')->result();
...
{
    $ci =& get_instance();
    $devices = $ci->db->select('device, COUNT(id) as total')->group_by('device')->get('student_logs')->result();
