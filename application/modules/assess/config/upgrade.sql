...
--- Date: 16th Aug, 2026 --- SCA support for scenario practice (peer practice mark scheme) ---

-- Applied to eprap_portal on 2026-08-16.
ALTER TABLE `scenario_topics_items_students`
  ADD `welcomes_patient` ENUM('Yes','No') NULL DEFAULT NULL COMMENT 'For SCA' AFTER `starts_station_well`,
  ADD `starts_with_open_end` ENUM('Yes','No') NULL DEFAULT NULL COMMENT 'For SCA' AFTER `welcomes_patient`,
  CHANGE `overall_judgment` `overall_judgment` ENUM('Fail','Borderline','Pass','Very Good','Excellent','Bare Pass','Bare Fail') NULL DEFAULT NULL,
  CHANGE `technical_skills` `technical_skills` DECIMAL(2,1) NULL DEFAULT NULL,
  CHANGE `clinical_skills` `clinical_skills` DECIMAL(2,1) NULL DEFAULT NULL,
  CHANGE `interpersonal_skills` `interpersonal_skills` DECIMAL(2,1) NULL DEFAULT NULL;

-- SCA statements ticked by the peer examiner in one scenario practice session.
CREATE TABLE IF NOT EXISTS `practice_feedback_statements` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `practice_id` BIGINT(20) NOT NULL COMMENT 'scenario_topics_items_students.id',
  `statement_id` INT UNSIGNED NOT NULL COMMENT 'sca_feedback_statements.id',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_practice_statement` (`practice_id`,`statement_id`),
  KEY `idx_statement_id` (`statement_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
