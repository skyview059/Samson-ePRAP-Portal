...
class Cron extends CI_Controller {
    ...
    public function alter(){
        ...
        $tables = $this->db->query(
            'SELECT CONCAT("ALTER TABLE `", TABLE_SCHEMA, "`.`", TABLE_NAME, "` DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci;") AS ExecuteTheString
            FROM INFORMATION_SCHEMA.TABLES
            WHERE TABLE_SCHEMA="eprap_portal"
            AND TABLE_TYPE="BASE TABLE" and table_collation != "utf8mb4_unicode_ci"'
        )->result();

        echo '<pre>';
        foreach($tables as $table ){
            echo $table->ExecuteTheString . "\r\n";            
        }
        echo '</pre>';
    }
