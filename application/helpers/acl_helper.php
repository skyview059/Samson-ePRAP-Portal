...
function checkPermission($access_key, $role_id) {
    $ci =& get_instance();
    $ci->db->cache_on();    
    $result = $ci->db->from('role_permissions')
...
function add_main_menu($title, $url, $access, $icon) {
    ...
    $active_url = get_instance()->uri->uri_string();
    $role_id = (int) getLoginUserData('role_id');
    $menu = '';
...
function top_nav($title, $url, $access, $icon) {
    // $title, $url, $icon, $access.
    $ci = & get_instance();
...
function addAdminMenu($name, $url = '', $icon = 'fa-envelope-o', $childrens = null) {
    ...
    $html = '';
    $access = checkMenuPermission($url, $role_id);
    if ( $access == 0 ) {
        return false;
    }
    ...
    $html .= '</a>';
    $html .= makeSubMenu($childrens,$role_id);        
    $html .= '</li>';
    
    return $html;
}

function makeSubMenu($childrens,$role_id){
    if(!$childrens){ return false; }
    
    $html = '<ul class="treeview-menu">';
    foreach ($childrens as $item) {
        $url = current( explode("?", $item['href']));
...
function addAdminChildMenu($title = 'Child Item', $href = 'admin', $icon = 'fa-circle-o') {
    $ci = & get_instance();
    ...
    $class_active = ( Backend_URL . $href == $active_url ) ? ' class="active"' : '';
    return "<li{$class_active}>"
            . '<a href="' . Backend_URL . $href . '">'
            . "<i class=\"fa {$icon}\"></i> {$title} </a></li>";
}
